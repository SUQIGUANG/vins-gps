...
/// GlobalOptimization类在构造过程就把优化做了，一开始还纳闷咋没找到哪做的优化
...
void GlobalOptimization::inputOdom(double t, Eigen::Vector3d OdomP, Eigen::Quaterniond OdomQ)
{
    ...
    vector<double> globalPose{globalP.x(), globalP.y(), globalP.z(),                 /// 位置
                              globalQ.w(), globalQ.x(), globalQ.y(), globalQ.z()};   /// 姿态
...
/**
 * 下面代码中出现了一个关键的部分，即WGPS_T_WVIO的计算。从之前的代码中知道，
 * 这个4*4矩阵是用来做VIO到GPS坐标系的变换的。按道理讲，这个矩阵应当是不需要
 * 反复计算的，毕竟第一帧和GPS的坐标变换是确定的。但是运行结果来看，这个矩阵的
 * 值是在变化的，而且有时变化比较大。这里不太懂，知乎刘知说是为了避免VIO漂移过大
 */
